Public Class ListeBeneficiairesNvCon
    ...
    Private Sub OnBddChanged()
        ChargerListeSuggestions()
        ' filtrage ?
        For n As Integer = ListeBenefsBdd.Count - 1 To 0 Step -1
            ' charger nouvelles infos pour chaque ctrlbenef
            Dim ctrlb = ListeBenefsBdd(n)
            Dim nvbenef = Bdd.Load(Of Acteur)(ctrlb.Benef.Id)
            If nvbenef IsNot Nothing Then
                ctrlb.Benef = nvbenef
            Else
                ' si il n'est plus là (supprimé), l'enlève de la liste des bénefs
                ' 'il pourrait être retransformé en contrôle éditable, mais soit)
                ctrlb.Parent.Controls.Remove(ctrlb)      ' dispose ?
                ListeBenefsBdd.RemoveAt(n)
            End If
        Next
    End Sub
    ...
    Public Sub ChargerListeSuggestions()
        Try
            LbSuggestions.DataSource = New SortableBindingList(Of IEntity.Condense)(Bdd.GetCondenseActeurs())
    ...
    Sub LdBenef() Handles LbSuggestions.DoubleClick
        ' récupérer l'id
        ' charger les données
        If LbSuggestions.IdSelectionne IsNot Nothing Then
            Dim LeBenef = Bdd.Load(Of Acteur)(LbSuggestions.IdSelectionne)
            Dim CtrlBenefBdd As New ElemListeBenefBdd
            RemplacerCtrlBenefParCtrlBenefBdd(ListeSuggestionEstAttacheeA, LeBenef)


            ' remplacer le "nouveau benef" par un benef bdd non éditable mais avec un bouton Modifier
            ' remettre le lien de parenté si présent
